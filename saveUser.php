		...
		elseif($passwd==$c_passwd){
			...
			if(!$conn){
					...
					if(mysqli_num_rows($s)==0){	// && mysql_num_rows($s)==0){
						...
						$q2.=",PASSWORD(\"$passwd\"),	FLOOR(RAND()*1000000))";
						...
						if(!$status){
							...
							//---------------------email ----
								...
								$body = "
										...
										<body>
										<p>".$d['name']." Sir, </br><h4>Your account is created in RExCS Successfully.</h4>
										<br> Username :<b>".$to."</b>
								...
								// More headers
								$headers .= "From: <" .$email.">" . "\r\n";
								if (mail($to, $subject, $body, $headers)) { $retStr= array(11,$d['email'],"Successfuly Created username: <b>$to</b>"); }
...
elseif($action=='change'){
		...
		if($passwd==$c_passwd){
			...
			if(!$conn){
					...
					$query="SELECT * FROM  $userTable
							WHERE 	id=\"$id\" AND passwd=PASSWORD(\"$cur_passwd\")";
					$status=mysqli_query($conn,$query);
					//-------------------------------------------------
					if(!(!$status) && mysqli_num_rows($status)>0){
						
						$q="UPDATE $userTable SET passwd=PASSWORD(\"$passwd\"), activationKey=FLOOR(RAND()*1000000) WHERE id=\"$id\"";
						...
						if(!$status){
							...
							$status=mysqli_query($conn,$query);
							$d=mysqli_fetch_array($status);
							$retStr= array(11,"","Password Changed <br> login with new credentials");// SUCCESS --> USER CREATED
						}
					}
					else{
	...
	elseif($action=='reset'){
			//$q1="INSERT INTO $userTable (rly_code,divn_code,username";
			$q="UPDATE $userTable SET passwd=PASSWORD('iriset'), activationKey=FLOOR(RAND()*1000000) WHERE id=\"$id\"";
