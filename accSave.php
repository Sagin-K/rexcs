...
if(isset($_GET['action'])){
	...
	//--------------------------
	if($action=="ip"){ 
						$fList	=array("server_id","acc_type","acc_name","icom_name","disp_name","icom_no","secret1","rly_no","pstn_no","phone","mac","updated_by");
						$tableName= $accTable; 
					}
	if($action=="analog"){ 
	...
	if($action=="reg"){ 
						$fList	=array("rly","divn","location","reg1_name","reg1_ip","reg2_ip","reg1_user","reg1_root","rly_no_length","icom_no_length","pstn_no_length","rly_code","pstn_code","updated_by");
						$tableName= $regTable; 
					}
	if($action=="icom"){ 
	...
	if($id=='' && ( $action=="ip" || $action=="analog" || $action=="reg" || $action=="icom" || $action=="route"|| $action=="gw"|| $action=="fxo"|| $action=="fxs"|| $action=="conf")){
...
//------------ Check ICOM and Delete Registrar ----------------------------------------------------------------------------------------------------
	...
	elseif($id!='' && $action=="reg" && $task=="delete"){
		...
		$q="DELETE FROM $accTable WHERE server_id=\"$id\"";
		//print $q;
		$s=mysqli_query($conn,$q);
		if(!$s ){
			$retStr=array(90,$id,'Not deleted.!! Some Accounts are available  which are not able to delete');
		}
		else{
		$q="DELETE FROM $icomTable WHERE server_id=\"$id\"";
		//print $q;
		$s=mysqli_query($conn,$q);
		if(!$s){
...
//----------------CLEAR COLUMNS ------------------------------------------------------------------------------------------------------------
	...
	elseif($id!="" && ($action=="ip2" || $action=="bs" || $action=="map" || $action=="byte") && $task=="delete"){
