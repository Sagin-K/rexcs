...
function validateLogRequest($u,$p){
	...
	$dbName="asterisk";	$tableName="astusers";
	//------------------------------------------------------
		...
		if(!$status){
			...
			if(mysqli_num_rows($status)>0){
				...
				elseif(strcasecmp($userData['accountStatus'],'blocked')==0){
					...
					//-------------------------------------------------
					$query="SELECT * FROM  $tableName
							WHERE 	email=\"$u\" AND passwd=PASSWORD(\"$p\") AND accountStatus=\"active\"";
					$status=mysqli_query($conn,$query);
					//-------------------------------------------------
					if(!(!$status) & mysqli_num_rows($status)>0){
						...
						$query="UPDATE $tableName SET last_log=NOW() WHERE id=\"{$data['id']}\"";
						$status=mysqli_query($conn,$query);
...
//-----------------------------------------------------------
		...
		$conn=mysqli_connect("localhost","root","12345",$dbName);
		$tableName="astusers";
		$query="SELECT id FROM $tableName WHERE name=\"".$_SESSION['astuser']."\"";
		$status=mysqli_query($conn,$query);
